//META{"name":"UserActivityImages"}*//

class UserActivityImages {
  ...
  constructor () {
    ...
    this.GAME_ICONS_24x24_BASE = 'https://twitchcord.github.io/discord/assets/images/gameicons/24x24/';
    this.observers = [];
  ...
  async injectUserActivities () {
    ...
    document
      .querySelectorAll('.member-3W1lQa')
      .forEach(member => {
        const activityTextNode = member.querySelector('.activityText-sLG0UL');
        if (activityTextNode) {
      ...
      changes.forEach(
        (change, i) => {
          if (change.removedNodes && change.removedNodes.length !== 0 &&
            change.removedNodes[0].classList && change.removedNodes[0].classList.contains('activity-1IYsbk') &&
            change.previousSibling && change.previousSibling.closest('.member-3W1lQa') &&
            change.previousSibling.closest('.member-3W1lQa').querySelector(`.${this.tcActivityPlaying}`) &&
            change.previousSibling.closest('.member-3W1lQa').querySelector(`.${this.tcActivityPlaying}`).parentElement) {
            change.previousSibling.closest('.member-3W1lQa').querySelector(`.${this.tcActivityPlaying}`).parentElement.removeChild(change.previousSibling.closest('.member-3W1lQa').querySelector(`.${this.tcActivityPlaying}`));
          }
          if (change.addedNodes) {
            change.addedNodes.forEach((node) => {
              if (node.querySelector) {
                let strong = node.querySelector('.activityText-sLG0UL strong');
                if (node && strong) {
          ...
          if (change.type && change.type == 'characterData' && change.target) {
            if (change.target.parentElement && change.target.parentElement.className == 'activityText-sLG0UL') {
  ...
  async injectChannelMembersActivities (node) {
    let returnString,
        ...
        memberParent = node.closest('.member-3W1lQa');
    ...
    for (const activity of this.activityList) {
      ...
      if (nodeString.includes(activityLowerCase)) {
        const slugifyReturnString = slugify(activity);
        ...
        if (tcStatusImage && tcStatusImage.childNode) {
          ...
          `;
          memberParent.insertAdjacentHTML('beforeend', returnDiv);
        }
        break;
      } else {
        if (tcStatusImage) {
